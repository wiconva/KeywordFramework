...
public class WebActions {
    ...
    public WebActions (String driverName, boolean headless){
        ...
        TLogger.trackeTest("Starting web driver ["+driverName+"]",TLogger.NORMAL_LEVEL);
        switch (driverName){
            case CHROME_DRIVER:
            ...
            case EDGE_DRIVER:
                ...
                break;
            default:
                TLogger.trackeTest("The web driver:["+driverName+"] is not supported",TLogger.ERROR_LEVEL );
                TestController.validateTest(TLogger.ERROR_LEVEL);
                break;
        }
        TLogger.trackeTest("Driver Started",TLogger.NORMAL_LEVEL);
    }

    public boolean browserGet(String objectWebStep[], String []inputStep, String [] outputSteps ){
        try{
            TLogger.trackeTest("Opening the brower", TLogger.NORMAL_LEVEL);
            String url = inputStep[0];
            this.webDriver.get(url);
            TLogger.trackeTest("The browser is Open in: "+ url, TLogger.NORMAL_LEVEL);
            return true;
        }catch(Exception e){
            TLogger.trackeTest("The URL resource is not disponible, check the URL",TLogger.ERROR_LEVEL);
    ...
    public boolean inputText (String objectWebStep[], String []inputStep, String [] outputSteps ){
        try{
            TLogger.trackeTest("Begin insert Text in the Web Element gived", TLogger.NORMAL_LEVEL);
            String text = inputStep[0];
            WebElement inputText = this.findWebElement(objectWebStep);
            inputText.sendKeys(text);
            TLogger.trackeTest("The text was insert in the input", TLogger.NORMAL_LEVEL);
            return true;
        }catch (Exception e){
            TLogger.trackeTest("Not was possible insert text in web element, check that Web Element is enable for action",TLogger.ERROR_LEVEL);
            TLogger.trackeTest(e.getMessage(), TLogger.ERROR_LEVEL);
            return false;
        }
    ...
    public boolean getText(String[] currentWebObjectStep, String[] currentInputStep, String[] currentOutputStep, Hashtable<String, String> outputsList) {
        try{
            TLogger.trackeTest("Getting the text from web element",TLogger.NORMAL_LEVEL);
            String textValue ="";
            WebElement we = findWebElement(currentWebObjectStep);
            textValue = we.getText().trim();
            outputsList.put(currentOutputStep[0],textValue);
            TLogger.trackeTest("The text value finded is: ["+textValue+"]",TLogger.NORMAL_LEVEL);
            return true;
        }catch (Exception e){
            TLogger.trackeTest("Not was possible get the text, check the attribute of Webelement", TLogger.ERROR_LEVEL);
            TLogger.trackeTest(e.getMessage(), TLogger.ERROR_LEVEL);
            return false;
        }
    ...
    private WebElement findWebElement (String [] objectWebStep) {
        try {
            TLogger.trackeTest("Searching the Web Element in the Web page", TLogger.NORMAL_LEVEL);
            ...
            TLogger.trackeTest("The web element is not in the DOM, check tne xpath is ok, or if loaded the web element ", TLogger.ERROR_LEVEL);
            TLogger.trackeTest(e.getMessage(), TLogger.ERROR_LEVEL);
            return null;
        }
    }
